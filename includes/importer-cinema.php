...
class Ktn_Cinema_Importer
{
    ...
    public static function syncCinema($post_id)
    {
        ...
        // Condition to autofill title: empty, numeric, Auto Draft, or fallback labels
        ...
        if ($is_autogenerated && !empty($best_display_name)) {
             // Remove action to prevent loop if save_post logic exists
        ...
        } elseif (!empty($metadata['notes']) && empty($current_post->post_content)) {
             // Just update content if title was already set but content is empty
             if (function_exists('ktn_cinema_save_all_meta_data')) {
                 remove_action('save_post_ktn_cinema', 'ktn_cinema_save_all_meta_data');
             }
             wp_update_post(array(
        ...
        if (!empty($metadata['logo'])) {
            // Check if logo is different before updating
            $old_logo = get_post_meta($post_id, '_ktn_cinema_logo', true);
            if ($old_logo !== $metadata['logo']) {
        ...
        // Handle City & Area (Taxonomy association)
        if (!empty($metadata['city'])) {
            $city_term = wp_set_object_terms($post_id, $metadata['city'], 'cinema_city', false);
            update_post_meta($post_id, '_ktn_cinema_city', sanitize_text_field($metadata['city']));
        }
        if (!empty($metadata['area'])) {
            $area_term = wp_set_object_terms($post_id, $metadata['area'], 'cinema_area', false);
            update_post_meta($post_id, '_ktn_cinema_area', sanitize_text_field($metadata['area']));
            
            // If area is set, we might want to ensure it has the city as parent if using hierarchical locs
            // But for now we use separate tax as requested or loose meta
        }
        ...
        if (!empty($metadata['maps_url'])) {
            update_post_meta($post_id, '_ktn_cinema_maps_url', esc_url_raw($metadata['maps_url']));
        }

        // Clean up and save showtimes to DB
        ...
        update_post_meta($post_id, '_ktn_last_error', 'Success: ' . $added . ' showtimes synced.');
        return $added;
    }
