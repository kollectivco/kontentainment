...
/**
 * Hook into gettext filter to dynamically override frontend strings using the Quick Translation dashboard entries.
 */
add_filter('gettext', 'ktn_quick_translate_filter', 20, 3);
function ktn_quick_translate_filter($translated, $text, $domain) {
    if (is_admin()) {
        return $translated;
    }
...
/**
 * Hook into ngettext filter to dynamically override plural frontend strings using the Quick Translation dashboard entries.
 */
add_filter('ngettext', 'ktn_quick_translate_nfilter', 20, 5);
function ktn_quick_translate_nfilter($translation, $single, $plural, $number, $domain) {
    if (is_admin()) {
        return $translation;
    }
    if ($domain === 'kontentainment') {
        $translations = get_option('ktn_quick_translations', array());
        if (isset($translations[$single]) && $translations[$single] !== '') {
            return $translations[$single];
        }
        if (isset($translations[$plural]) && $translations[$plural] !== '') {
