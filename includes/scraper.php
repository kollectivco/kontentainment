...
require_once KTN_PLUGIN_DIR . 'includes/parsers/scenenow-parser.php';
require_once KTN_PLUGIN_DIR . 'includes/parsers/cairo360-parser.php';
    ...
    public static function fetch_from_source($cinema_id, $url, $source_type = 'elcinema_theater')
    {
        ...
        if ($source_type === 'elcinema_theater' || strpos($url, 'elcinema.com') !== false) {
             ...
             if (preg_match_all('/href="([^"]*[\?&]date=([0-9]{4}-[0-9]{1,2}-[0-9]{1,2}))"/i', $html, $matches)) {
                 foreach (array_unique(array_slice($matches[1], 0, 7)) as $rel_url) {
                     $date_url = (strpos($rel_url, 'http') === 0) ? $rel_url : 'https://elcinema.com' . (strpos($rel_url, '/') === 0 ? '' : '/') . $rel_url;
                     if (strpos($url, '/en/') !== false && strpos($date_url, '/en/') === false) $date_url = str_replace('elcinema.com/', 'elcinema.com/en/', $date_url);
                     if ($date_url === $url) continue;
                     usleep(300000); 
                     $d_res = wp_remote_get($date_url, $args);
                     if (!is_wp_error($d_res)) {
                         $d_data = Ktn_Elcinema_Parser::parse(wp_remote_retrieve_body($d_res), $date_url, $cinema_id, $scraped_at);
                         if (!empty($d_data['showtimes'])) $results = array_merge($results, $d_data['showtimes']);
                     }
                 }
             }
        } elseif ($source_type === 'scenenow_cinema' || strpos($url, 'scenenow.com') !== false) {
             $initial = Ktn_Scenenow_Parser::parse($html, $url, $cinema_id, $scraped_at);
             $results = $initial['showtimes']; $metadata = $initial['metadata'];
             // SceneNow Multi-date (look for option values in select#area)
             if (preg_match_all('/<option[^>]*value="([^"]*)"/is', $html, $matches)) {
                 foreach (array_unique(array_slice($matches[1], 0, 7)) as $date_label) {
                      // SceneNow often requires AJAX or different URL for dates, if it's just a label we might skip or try to append
                      // For now, if current parser found data for the "Active" one, we continue.
                 }
             }
        } elseif ($source_type === 'cairo360_cinema' || strpos($url, 'cairo360.com') !== false) {
             $initial = Ktn_Cairo360_Parser::parse($html, $url, $cinema_id, $scraped_at);
             $results = $initial['showtimes']; $metadata = $initial['metadata'];
        } else {
             return new WP_Error('invalid_source', 'Unsupported source type or pattern.');
        }
